...
public:
    ...
    void connect(data_cache_t *m_cache);                    // Connect to upper-level cache.
    void load_block(uint64_t m_addr, uint64_t m_block_size);// Load memory block.
    void run();                                             // Run data memory.
    void print_state() const;                               // Print memory state.

private:
    void load_memory_state();                               // Load initial memory state.

    data_cache_t *cache;                                    // Pointer to upper-level cache
