...
Supports for non-doubleword memory accesses such as {\tt lb}, {\tt lhu}, {\tt sw} have been added to Kite, as shown in the table above.
There are two pairs of load instructions (i.e., signed, unsigned) for each of byte, halfword, and word-granular loads.
Unlike load instructions, store instructions do not specify signs but only data lengths.
Several basic floating-point instructions also have been added to the list of supported instructions including {\tt fadd.d}, {\tt fdiv.d}, {\tt fmul.d}, and {\tt fsub.d}, where the tailing {\tt .d} indicates that the instructions are double-precision.
Memory instructions for double-precision data (i.e., {\tt fld}, {\tt fsd}) are supported as well.
Supports of other floating-point instructions including single-precision data type (e.g., {\tt fadd.s}, {\tt flw}) remain as a future work.
...
Every address in the memory state file must be a multiple of 8 for 64-bit granular data alignment.
If a value is given in a floating-point format such as {\tt 1.23} or {\tt 3.4e5}, it will saved in the memory address as the double-precision data format.
